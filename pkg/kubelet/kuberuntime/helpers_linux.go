...
func sharesToMilliCPU(shares int64) int64 {
	...
	if shares >= int64(cm.MinShares) {
		// Fix: Use floor for MinShares to prevent CPU inflation in AllocatedResources
		// When containers have 0 CPU requests, they get MinShares (2), and math.Ceil(2*1000/1024) = 2m
		// This causes artificial inflation. Using floor for MinShares: floor(2*1000/1024) = 1m
		if shares == int64(cm.MinShares) {
			milliCPU = int64(float64(shares*cm.MilliCPUToCPU) / float64(cm.SharesPerCPU))
		} else {
